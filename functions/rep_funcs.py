...
class ReputationDB:
	...
	def change_rep(self, user_id: int, new_rep: int, guild_id: str):
		self.reputation = self.db[guild_id]
	...
	def add_rep(self, user_id: int, guild_id: str, date):
		self.reputation = self.db[guild_id]
		self.reputation.update_one({"id": user_id}, { "$inc": {"rep": rep}, "$push": {"history": {"date": date, "rep": 1}} }, upsert=True)
		rep = self.reputation.find_one({"id": user_id})
		return rep["rep"]

	def rep_leaderboard(self, guild_id: str):
		self.reputation = self.db[guild_id]
		leaderboard = self.reputation.find().sort("rep", -1)
