...
class Reputation(commands.Cog):
	...
	async def on_message(self, message):
		if functions.preferences.gpdb.get_pref('rep_enabled', message.guild.id):
			await functions.rep_funcs.repMessages(message)  # If message is replying to another message
	...
	async def rep(self, interaction: discord.Interaction,
		...
		embed = discord.Embed(title=f"{user.display_name}'s Reputation", description=f"{user} has {rep} rep.")
	...
	async def change_rep(self, interaction: discord.Interaction,
		...
		if await functions.utility.isModerator(interaction.user):
			await interaction.response.defer()
			rep = self.repDB.change_rep(user.id, new_rep, str(interaction.guild.id), interaction.created_at)
	...
	async def leaderboard(self, interaction: discord.Interaction,
		...
		await interaction.response.defer()
		leaderboard = self.repDB.rep_leaderboard(str(interaction.guild.id))  # Rep leaderboard
