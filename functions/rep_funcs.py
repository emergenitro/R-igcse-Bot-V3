...
class ReputationDB:
    ...
    def bulk_insert_rep(self, rep_dict: dict, guild_id: str, date):
        ...
        for user_id, rep in user_reps.items():
            result = self.reputation.update_one({"id": user_id}, {"$inc": {"rep": rep}, "$push": {
                                                "history": {"date": date, "rep": rep}}}, upsert=True)
    ...
    def change_rep(self, user_id: int, new_rep: int, guild_id: str, date):
        ...
        if current_rep == None:
            ...
            rep_gained = new_rep - current_rep
        self.reputation.update_one({"id": user_id}, {"$set": {"rep": new_rep}, "$push": {
                                   "history": {"date": date, "rep": rep_gained}}}, upsert=True)
        return new_rep

    def delete_user(self, user_id: int, guild_id: str):
        self.reputation = self.db[guild_id]
        result = self.reputation.delete_one(
            {"id": user_id, "guild_id": guild_id})
        return result

    def add_rep(self, user_id: int, guild_id: str, date):
        self.reputation = self.db[guild_id]
        self.reputation.update_one({"id": user_id}, {"$inc": {"rep": 1}, "$push": {
                                   "history": {"date": date, "rep": 1}}}, upsert=True)
...
async def repMessages(message):
    ...
    if repped:
        for user in repped:
            rep = repDB.add_rep(user.id, str(
                message.guild.id), message.created_at)
        ...
        # Creating list of Reputed member ids
