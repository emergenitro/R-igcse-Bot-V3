...
class Reputation(commands.Cog):
    ...
    async def rep(self, interaction: discord.Interaction,
        ...
        rep = self.repDB.get_rep(user.id, interaction.guild.id)
        if rep is None:
            rep = 0
    ...
    async def change_rep(self, interaction: discord.Interaction,
        ...
        if await functions.utility.isModerator(interaction.user):
            await interaction.response.defer()
            rep = self.repDB.change_rep(user.id, new_rep, interaction.guild.id)
    ...
    async def leaderboard(self, interaction: discord.Interaction,
        ...
        await interaction.response.defer()
        leaderboard = self.repDB.rep_leaderboard(interaction.guild.id)  # Rep leaderboard
        ...
        for n, chunk in enumerate(chunks):
            ...
            for user, rep in chunk:
                ...
                if rep == 0 or user_name is None:
                    self.repDB.delete_user(user, interaction.guild.id)
                else:
